...
- (void)viewDidLayoutSubviews {
    ...
    g_playView.frame = CGRectMake(0, 0, ScreenWidth, 300);
    ...
    [g_qualityPanel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(ScreenWidth, 74));
    }];
...
- (void)talking:(UIButton *)sender {
    ...
    if (g_playView.isTalking) {
        ...
        if (!self.talkChannelView) {
            self.talkChannelView = [[TalkChannelView alloc]  initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
    ...
    if (show) {
        if (!g_ptzPopView) {
            ...
            if ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationPortrait) {
                g_ptzPopView.center = CGPointMake(ScreenWidth/2,g_playView.center.y + 100);
            }
        }
...
- (void)showPtzPresetPositionPopView:(BOOL)show {
    if (show) {
        if (!g_ptzPresetPositionPopView) {
            ...
            if ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationPortrait) {
                g_ptzPresetPositionPopView.center = CGPointMake(ScreenWidth/2,g_playView.center.y + 100);
            }
