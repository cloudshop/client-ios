...
- (void)initialize {
    ...
    UIColor *borderColor = [UIColor colorWithRed:255.0f/255.0f green:255.0f/255.0f blue:255.0f/255.0f alpha:1.0f];
    self.layer.borderColor = [borderColor CGColor];//[[UIColor lightGrayColor] CGColor];
...
- (void)layoutSubviews {
    ...
    for (int i = 0; i < [self.segments count]; i++) {
        ...
        CGSize titleSize = [segment.titleLabel.text sizeWithFont:self.selectedTitleFont constrainedToSize:CGSizeMake(MAXFLOAT, segmentHeight)];
        segment.lineView.frame=CGRectMake((segmentWidth-titleSize.width)/2, segmentHeight-2.5, titleSize.width, 2.5);
        if (self.stylesTitleForSelectedSegment && self.selectedSegmentIndex == i) {
            ...
            segment.lineView.backgroundColor=self.selectedTitleTextColor;//[UIColor colorWithRed:101.0/255 green:94.0/255 blue:230.0/255 alpha:1.0];
        } else {
            segment.titleLabel.font = self.titleFont;
            segment.titleLabel.textColor = self.titleTextColor;
            segment.lineView.backgroundColor=[UIColor clearColor];
        }
    }
...
- (void)moveSelectedSegmentIndicatorToSegmentAtIndex:(NSUInteger)index animated:(BOOL)animated {
    ...
    if (index != self.selectedSegmentIndex) {
        ...
        previousSegment.lineView.backgroundColor=[UIColor clearColor];
        ...
        [UIView animateWithDuration:self.segmentIndicatorAnimationDuration
                         ...
                         completion:^(BOOL finished) {
                             if (self.stylesTitleForSelectedSegment) {
                                 ...
                                 selectedSegment.lineView.backgroundColor=self.selectedTitleTextColor;
