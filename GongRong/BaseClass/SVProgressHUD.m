...
+ (void)show {
    ...
    [self performSelector:@selector(cancelProgressHUD) withObject:nil afterDelay:7.0f];
}

+ (void)showInCrossScreen {
    [[SVProgressHUD sharedView] showWithStatus:@"加载中" maskType:SVProgressHUDMaskTypeGradient networkIndicator:NO];
   // [SVProgressHUD sharedView].hudView.center=CGPointMake(2, ScreenWidth);
   // [[SVProgressHUD sharedView] moveToPoint:CGPointMake(ScreenWidth/2, ScreenHeight/2) rotateAngle:M_PI/2.0f];
    
    [SVProgressHUD sharedView].hudView.backgroundColor=kAppColor6;
   // [[SVProgressHUD sharedView] setNeedsDisplay];
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    switch (orientation) { 
        ...
        case UIInterfaceOrientationLandscapeRight:
            ...
            newCenter = CGPointMake(orientationFrame.size.width/2.0, orientationFrame.size.height/2.0);
            break;
...
+ (BOOL)isUnvisible {
    return ([SVProgressHUD sharedView].alpha == 0);
}
